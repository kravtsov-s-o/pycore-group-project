...
from helpers.Serialization import Serialization
...
class AddressBook:
    ...
    def add_record(self, user_input):
        ...
        (
            ...
            phone,
            *args,
        ) = commands  # Розпакування перших двох значень та всіх інших у змінну args
    ...
    def find_records(self, searching_str: str):
        result = [
            record
            for record in self.records
            if searching_str in f"{record.name._value} {record.phone._value} "
    ...
    def get_upcoming_birthday_contacts(self, days):
        ...
        for record in self.records:
            days_to_birthday = (
                record.birthday.days_to_birthday() if record.birthday else ""
            )
