...
def init():
    ...
    addressbook_manager = AddressBookManager(
        AddressBook(FILE_CONTACTS), ui_manager, ITEMS_PER_PAGE
    )
    notes_list_manager = NotesListManager(
        NotesList(FILE_NOTES), ui_manager, ITEMS_PER_PAGE
    )
    file_sorter = FileSorter(ui_manager)

    app = ConsoleApplication(
        ui_manager, addressbook_manager, notes_list_manager, file_sorter
    )
    app.run()
